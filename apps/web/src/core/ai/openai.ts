...
// key is absent; it only fails when an analysis is actually requested. Exported
// so the Ask Atlas agent (ask-agent.ts) reuses the same configured client.
...
// Ask Atlas follow-up (ATLAS-010) now runs as a bounded tool-calling agent; see
// answerQuestionAgentic in ask-agent.ts.
